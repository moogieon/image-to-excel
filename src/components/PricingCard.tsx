      ...
      <div className="grid md:grid-cols-3 gap-5">
        {plans.map((plan) => (
          <div
            key={plan.name}
            className={`
              rounded-[24px] p-6 relative flex flex-col
              ${plan.highlight
                ? 'liquid-glass-accent liquid-glass-hover md:-mt-2 md:mb-[-8px]'
          ...
          >
            {plan.highlight && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <span className="text-[11px] font-semibold text-white px-3.5 py-1 rounded-full bg-primary/90 backdrop-blur-sm">
                  Popular
                </span>
              </div>
            )}
            ...
            <div className="text-center mb-6 pt-2">
              <div className="w-10 h-10 mx-auto rounded-xl bg-black/6 flex items-center justify-center mb-4">
              ...
              <h3 className="text-[17px] font-bold tracking-tight text-text">{plan.name}</h3>
              <p className="text-[13px] text-text-muted mt-1">{plan.desc}</p>
              <p className="text-[28px] font-bold mt-4 tracking-tight text-text">{plan.price}</p>
            </div>
              ...
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-center gap-2.5 text-[14px] text-text-secondary">
                  <div className="w-5 h-5 rounded-full bg-primary/15 flex items-center justify-center shrink-0">
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#217346" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            ...
            <button className="w-full py-2.5 rounded-2xl font-semibold text-[14px] btn-primary mt-6">
              {plan.cta}
            </button>
          </div>
        ))}
